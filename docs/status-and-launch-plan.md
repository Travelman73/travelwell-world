...
- **Integration read (what's real vs socket today):** the **speak/type → Atlas captures → writes the vision back → confirm** loop is genuinely built (`useSpeech` + `atlas/index.ts`); age-*ranges* + Travel I.D. persist to the DB and resume across devices. **Sockets to shape, not build now:** (a) promote the profile from single-trip to a **permanent Identity Card** (the `constant`), with the vision stored separately as per-trip intent; (b) **per-Well budget as up-to-3 ranges** (today budget is coarser) — a small schema widening, not a rewrite; (c) the capabilities/limitations overlay as structured, safety-linked fields (feeds the L3 gates); (d) the **Loop check-in** as a returning-session diff over the stored ID. All ride the existing profile schema + the "subject-not-user" seam — no fork. **VC ask (#7):** the whole **Sign-up + Sign-in + full Onboarding** should be a first-class demo surface — it's the clearest "not like every other travel site" moment.
