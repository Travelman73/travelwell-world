...
> **⚠️ Universe set corrected — David 2026-08 (supersedes the Sports/Olympics framing below).** The universes are now **four front doors: TravelWell (general) · TravelWell-Ultra · Romance (Marriages & Honeymoons) · TLEU / Live Entertainment**, with **Adventure *probable* but not locked** (its case rides `safari` + `expedition`, both high-commission/premium). **Olympics and team-Sports travel are OUT as universes** — David is deliberately staying clear of athlete/team movement (a 75-yr US near-monopoly). **The sports play is a single Signature Interest: `spectator`** — ticket + travel for *spectators* (pro football's ~8–15% of a 65k-avg gate travels; plus colleges, marathons, Ironman), an underserved lane we can actually work. So: no Olympics/CCSTT/team-travel universe build; `olympic`/`sports`/`prosports`/`compsports` SIs are **parked** (leave as `preview`, don't invest); `spectator` is the one to grow. The **subject-not-person schema seam stays** (it serves Romance wedding-blocks / groups too) — just no longer "Olympic"-driven. Reconcile the paragraphs below against this note.
...
- **Four front doors (David-confirmed, 2026-08):** TravelWell (general) · Ultra · Romance (Marriages & Honeymoons) · TLEU / Live Entertainment — each a themed layer over an SI/tier slice of the one tank. **Adventure is a probable fifth** (rides `safari` + `expedition` — premium, high-commission), not yet locked. **Not universes:** Olympics and team-Sports travel (dropped); the sports play is the `spectator` SI only. Marginal cost of the next door is a fraction of the first; that compounding is the moat.
