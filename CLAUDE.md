...
Not built yet, but the foundation is shaped to receive them clean (an institutional/entity universe — a team, a federation, a wedding block — plugs in with no rework). **Universe set (David-confirmed 2026-08): four front doors — TravelWell (general) · Ultra · Romance · TLEU/Live Entertainment (+ Adventure probable). Olympics and team-Sports travel are NOT universes; the sports play is the `spectator` SI only.** The subject-not-person seam stays (it also serves Romance wedding-blocks/groups). Detail in `docs/status-and-launch-plan.md`.
